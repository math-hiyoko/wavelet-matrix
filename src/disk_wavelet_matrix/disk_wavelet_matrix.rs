...
mod tests {
    ...
    fn create_u8() -> DiskWaveletMatrix<u8> {
        ...
        file.set_len((elements.len() * mem::size_of::<u8>()) as u64)
            .unwrap();
        #[allow(unsafe_code)]
        let mut mmap = unsafe { MmapMut::map_mut(&file).map_err(PyOSError::new_err).unwrap() };
    ...
    fn test_empty() {
        ...
        let file = tempfile().map_err(PyOSError::new_err).unwrap();
        #[allow(unsafe_code)]
        let mmap_empty = unsafe { MmapMut::map_mut(&file).map_err(PyOSError::new_err).unwrap() };
        ...
        assert_eq!(wv_u128.range_sum(0, 0).unwrap(), BigUint::zero(),);
        assert_eq!(
            wv_u128.range_intersection(0, 0, 0, 0).unwrap(),
            Vec::<(u128, usize, usize)>::new(),
        );
        assert_eq!(wv_u128.range_freq(0, 0, None, None).unwrap(), 0,);
        assert_eq!(
            wv_u128.range_list(0, 0, None, None).unwrap(),
            Vec::<(u128, usize)>::new()
        );
        assert_eq!(wv_u128.prev_value(0, 0, None).unwrap(), None,);
        assert_eq!(wv_u128.next_value(0, 0, None).unwrap(), None,);
    }
    ...
    fn test_all_zero() {
        ...
        #[allow(unsafe_code)]
        let mmap_u8_all_zero =
            unsafe { MmapMut::map_mut(&file).map_err(PyOSError::new_err).unwrap() };
