...
@pytest.fixture()
def wm_small():
    return WaveletMatrix([5, 4, 5, 5, 2, 1, 5, 6, 1, 3, 5, 0], on_disk=True)
...
def test_large_construction():
    ...
    with pytest.raises(
        OverflowError,
        match="Values must be u128 or smaller for on-disk WaveletMatrix",
    ):
...
def test_empty():
    ...
    wv_empty = WaveletMatrix([], on_disk=True)
...
def test_all_zero():
    ...
    wv_all_zero = WaveletMatrix([0] * 128, on_disk=True)
